"""Curriculum RAG for ClassroomAI — loaded from cbse_toc.json
(parsed from GRADE Wise TOC.xlsx: 12 grades, all subjects, ~769 chapters).

Used to ground every generator's output in the official curriculum.
"""
import json
import os
import re
...
def get_subjects(grade_level) -> list:
    return list(CURRICULUM_KB.get(_grade_key(grade_level), {}).keys())


def get_chapters(grade_level, subject) -> list:
    return CURRICULUM_KB.get(_grade_key(grade_level), {}).get(subject, [])
...
def retrieve_context(query, grade_level, subject="", top_k=3) -> str:
    """Find the most relevant chapters for a topic/query and return a
    formatted context block to ground the generator prompt."""
    gk = _grade_key(grade_level)
    grade_data = CURRICULUM_KB.get(gk, {})
    if not grade_data:
        return ""
    ...
    lines = [f"OFFICIAL {gk} CURRICULUM CONTEXT (align content to this):"]
